...
class LearningAgent:
    ...
    def learn(self,ost,nst,a,r):
        ...
        self.EXPLORATION_RATE = min(0.2, self.EXPLORATION_RATE * 0.99)
        ...
        for outcome in currentModel: # Iterate over "memorized" (state, reward) outcomes and compute a weighted sum
            ...
            totalMaxQ += self.maxQ(outcome['state'])
        self.qNodes[ost][a].q = (totalReward + self.GAMMA * totalMaxQ) / self.N[ost][a]
        ...
        while len(self.queue) and updates < self.MAX_UPDATES:
            ...
            for ancState, ancAction in random.sample(self.ancestors[state], min(len(self.ancestors[state]), 3)): # Iterate over (state, action) pairs that lead to current state
                # Update Q-value
                ...
                for outcome in self.model[ancState][ancAction]:
                    totalReward += outcome['reward']
                    totalMaxQ += self.maxQ(outcome['state'])
                self.qNodes[ancState][ancAction].q = (totalReward + self.GAMMA * totalMaxQ) / self.N[ancState][ancAction]
    ...
    def maxQ(self, st):
        return self.qLists[st].maxQ if self.visited[st] else 0
